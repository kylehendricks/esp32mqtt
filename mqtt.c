...
void deliver_publish(mqtt_client * client, uint8_t * message, int length)
{
  ...
  do {
    ...
    len_read = client->settings->read_cb(client,
                                         client->mqtt_state.in_buffer,
                                         CONFIG_MQTT_BUFFER_SIZE_BYTE,
                                         0);
    if (len_read < 0) {
...
void mqtt_start_receive_schedule(mqtt_client * client)
{
  ...
  while (1) {
    ...
    mqtt_info("msg_type %d, msg_id %d, pending_type %d, pending_id %d",
              msg_type, msg_id,
              client->mqtt_state.pending_msg_type,
              client->mqtt_state.pending_msg_id);
    switch (msg_type) {
      ...
      case MQTT_MSG_TYPE_PUBLISH:
        if (msg_qos == 1) {
          client->mqtt_state.outbound_message = mqtt_msg_puback(&client->mqtt_state.mqtt_connection,
                                                                msg_id);
        }
        else if (msg_qos == 2) {
          client->mqtt_state.outbound_message = mqtt_msg_pubrec(&client->mqtt_state.mqtt_connection,
                                                                msg_id);
        }
      ...
      case MQTT_MSG_TYPE_PUBREC:
        client->mqtt_state.outbound_message = mqtt_msg_pubrel(&client->mqtt_state.mqtt_connection,
                                                              msg_id);
        mqtt_queue(client);
        break;
...
void mqtt_subscribe(mqtt_client * client, const char * topic, uint8_t qos)
{
  client->mqtt_state.outbound_message = mqtt_msg_subscribe(&client->mqtt_state.mqtt_connection,
                                                           ...
                                                           qos,
                                                           &client->mqtt_state.pending_msg_id);
  mqtt_info("Queue subscribe, topic\"%s\", id: %d",
            topic,
            client->mqtt_state.pending_msg_id);
